...
export function generateColumnList(columns: SelectQuery['columns']): string[] | string {
  ...
  const columnsList = columns.map(col => {
    ...
    const { table, colName } = parseColumnName(name!)
    switch (type) {
...
export function generateFromString(
  ...
  const tableString =
    join?.length || tableName ? `${fromString} as ${tableName || 't0'}` : `${fromString}`
  return tableString
}
...
export function formatSql(params: {
  ...
  try {
    ...
    const tableString = generateFromString(from, fromS3, join)
    const appendedColumnList = groupby
      ? [...(columns || []), { name: groupby, type: 'Text', description: '' } as ColumnSchema]
      : columns!
    const columnList = generateColumnList(appendedColumnList)
    const statement = QueryBuilder('query').select(columnList).fromRaw(tableString)

    limit && statement.limit(limit)
    offset && statement.offset(offset)
    join &&
      join.forEach((join: JoinQuery, i) => {
        const { from, fromS3, leftOn, rightOn, operator } = join
        const tableString = generateFromString(from, fromS3, [], `t${i + 1}`)
        const { table: lOnTable, colName: lOnColumn } = parseColumnName(leftOn!)

        const { table: rOnTable, colName: rOnColumn } = parseColumnName(rightOn!)
