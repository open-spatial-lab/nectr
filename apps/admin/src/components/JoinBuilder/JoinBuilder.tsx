        ...
        <NoPaddingGrid style={{ width: "100%" }}>
            {joins.map((join, index) => {
                ...
                const rightColumn = rightSource?.columns.find(
                    column => column.name === join.rightOn
                );
                    ...
                    <React.Fragment key={`${join.leftSourceId}${join.rightSourceId}${index}`}>
                        <Cell span={4}>
                            <SourceSelector
                                ...
                                value={leftSource}
                                onChange={source =>
                                    updateJoinAtIndex(index, "leftSourceId", source.id)
                                }
                            />
                            <Stack direction="row" spacing={1} paddingTop={2} alignItems={"center"}>
                                ...
                                <ColumnSelect
                                    ...
                                    onChange={column =>
                                        updateJoinAtIndex(index, "leftOn", column.name)
                                    }
                                />
                            </Stack>
                        </Cell>
                        <Cell span={4}>
                            <SourceSelector
                                ...
                                onChange={source =>
                                    updateJoinAtIndex(index, "rightSourceId", source.id)
                                }
                            />
                            <Stack direction="row" spacing={1} paddingTop={2} alignItems={"center"}>
                                <IndentArrow />
                                <ColumnSelect
                                    ...
                                    onChange={column =>
                                        updateJoinAtIndex(index, "rightOn", column.name)
                                    }
                                />
                            </Stack>
                        </Cell>
                        <Cell span={3}>
                            <div style={{ width: "100%" }}>
                                {JOIN_OPERATORS.map(joinOp => {
                                    const Button =
                                        join.operator === joinOp ? ButtonPrimary : ButtonSecondary;
                                    const Icon = ICON_MAPPING[joinOp as keyof typeof ICON_MAPPING];
                                    return (
                                        <Button
                                            key={joinOp}
                                            onClick={() =>
                                                updateJoinAtIndex(index, "operator", joinOp)
                                            }
                                        >
                                            <div
                                                style={{
