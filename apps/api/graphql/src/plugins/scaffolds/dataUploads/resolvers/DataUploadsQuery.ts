  ...
  implements DataUploadsQuery
{
  ...
  initPermission() {
    this.permission = new Authorize<DataUploadsPermission>(this.context.security, 'data-uploads')
  }
  ...
  async listDataUploads({ limit = 10, sort, after, before }: ListDataUploadsParams) {
    ...
    const data = this.permission?.authorizeList<DataUploadEntity>(full, 'canView')
