...
export default [
    ...
    // The `PbEditorPageElementAdvancedSettingsPlugin` plugin.
    {
        ...
        render({ data, Bind, submit }) {
            ...
            const currentDataview = dataViews?.find(
                (item: any) => item.id === data.variables.source
            );
            const currentColumns = currentDataview
                ? JSON.parse(currentDataview?.template)?.columns
                : [];
                        ...
                        <Cell span={12}>
                            <Bind name="variables.layerType">
                                <Select
                                    label={"Map type:"}
                                    description={
                                        "Choose the type of map you would like to display."
                                    }
                                >
